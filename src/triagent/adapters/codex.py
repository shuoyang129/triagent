...
from triagent.adapters._cli import invoke_codex_jsonl, probe, restricted_input, runtime
...
class CodexAdapter(AgentAdapter):
    ...
    def run(self, request: AgentRequest) -> AgentResult:
        with restricted_input(request) as (path,error):
            ...
            return invoke_codex_jsonl(self._runner, [*self._command,"exec","--sandbox","read-only","--json","--input-file",str(path)], request.workdir, request.timeout_seconds, self._env, self._secrets, request.role)
