...
The normal DGX non-fake route uses Cursor Composer 2.5 Fast with Auto-review and sandboxing enabled, then Codex verification and independent Antigravity review. The OpenCode-backed DeepSeek fallback remains disabled in the checked-in DGX profile and defaults to `deepseek/deepseek-v4-pro`; it is not a Cursor custom model. When explicitly enabled, read `DEEPSEEK_API_KEY` only from the environment. The TriAgent OpenCode agent denies shell, network, subagents, skills, external directories, `.env`, and `.git`; do not weaken these permissions because a global OpenCode configuration is more permissive. If the operator stores the key in `~/.env`, export it into the TriAgent process environment without placing its value in argv or logs. Read only the allowlisted readiness diagnostic code; never persist or repeat provider response text. Treat provider failures as recoverable only when TriAgent says so; inspect the saved diagnostic and resume using the same profile digest and execution provenance. DGX/LAN/Isaac remains Phase B.
